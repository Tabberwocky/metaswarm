...
const TOML_COMMAND_MAP = {
  ...
  'pr-shepherd': {
    description: 'Monitor a PR through CI and review comments up to the merge decision',
    prompt: 'Invoke the metaswarm pr-shepherd skill. The user wants to shepherd a PR up to the merge decision.\n\nArguments provided by the user: {{args}}\n\nMonitor CI status, handle review comments, give every review thread a disposition (fixed with a reply, or declined with the reason in a reply), then hand back the one-line verdict "Ready to merge (my assessment): YES — … / NO — …". Merge only on the user\'s explicit instruction; never enable auto-merge unless asked.'
  },
  'brainstorm': {
  ...
  'status': {
    ...
    prompt: 'Invoke the metaswarm status skill. The user wants to check their installation status.\n\nArguments provided by the user: {{args}}\n\nRun the platform-aware diagnostic checks: plugin version, project setup, platform install state, command shims where applicable, legacy install, BEADS, bd CLI, external tools, coverage thresholds, Node.js.'
  },
  'handle-pr-comments': {
    description: 'Handle PR review comments with the full resolution workflow',
    prompt: 'Invoke the metaswarm handling-pr-comments skill. The user wants to address PR review comments.\n\nArguments provided by the user: {{args}}\n\nFetch inline comments and review bodies, handle "outside diff range" comments, and give every review thread a disposition (fixed with a reply, or declined with the reason in a reply; resolving is at your discretion). After a fix round that changed behavior, re-trigger only the bots that round owes, then re-check for new comments; repeat until none remain.'
  },
  'create-issue': {
  ...
  if (!fs.existsSync(codexMarketplacePath)) {
    ...
    try {
      ...
      if (!entry) {
        ...
        const isLocalForkSource = entry.source && entry.source.source === 'local' && entry.source.path === '.';
        if (!isLocalForkSource) {
          console.error('INVALID: metaswarm Codex marketplace source must point at the local fork root with {"source":"local","path":"."}');
          issues++;
        }
